...
class EnterRaffle:
    ...
    def fetch_raffle(self):
        ...
        while error_count < self.max_retries:
            try:
                response = self.session.get(
                    self.task.input.raffle["url"].replace("/fr/", "/en/"),
                    headers={
                        "cache-control": "max-age=0",
                        "sec-ch-ua": None,
                        "sec-ch-ua-mobile": "?0",
            ...
            if response.ok:
                if "/263-men" in response.url or "/264-women" in response.url:
                    self.logger.error("Failed to enter raffle: Raffle closed")
                    return False
                elif "Waiting the draw" in response.body:
                    self.logger.error("Failed to enter raffle: Not live yet")
                    return False

                try:
                    self.product.name = html.unescape(re.findall('"content_name":"(.*?)",', response.body)[0])
                    self.product.price = re.findall('itemprop="price" content=".*?">(.*?)<', response.body)[0]
                    self.product.image = re.findall('property="og:image" content="(.*?)"', response.body)[0]
                    self.product.size, self.data["sizeId"] = self.get_size(
                        re.findall(r'<li class="input-container ">([\s|\S]*?)</li>', response.body)
                    )

                    self.data["productId"] = re.findall('name="id_product" value="(.*?)"', response.body)[0]
                    self.data["sizeGroup"] = re.findall('data-product-attribute="(.*?)"', response.body)[0]
                    break
    ...
    def enter_raffle(self):
        ...
        while error_count < self.max_retries:
            try:
                response = self.session.post(
                    f"https://{DOMAIN}/en/launch-ajax?ajax=1",
                    body={
                        "recaptcha": self.captcha.solve(),
                        "id_product": self.data["productId"],
                        "id_attribute_group": self.data["sizeGroup"],
                        "attribute_group_value": self.data["sizeId"],
                        "instagram_account": self.task.parent.instagram
                    },
                    headers={
                        "content-length": None,
                        "sec-ch-ua": None,
                        ...
                        "referer": self.task.input.raffle["url"].replace("/fr/", "/en/"),
                        "accept-encoding": "gzip, deflate, br",
                        "accept-language": None
                    }
                )
